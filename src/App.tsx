...
function App() {
    ...
    return (
        <div className="h-screen w-full">
            <div
                className="w-2/3 h-full flex items-center justify-center m-auto overflow-x-auto overflow-y-auto relative shadow-md sm:rounded-lg">
                <table className="w-full align-center text-center text-lg text-left text-gray-500 justify-center mb-20">
                    {create === 'create' ?
                        <CreateUser removeModal={removeModal} setUsers={setUsers} users={users}/> : null}
                    {edit === 'edit' ? <EditUser removeModal={removeEditModal} editUser={editUser} setUsers={setUsers}
                                                 users={users}/> : null}
                    <thead className="text-xs text-gray-800 uppercase bg-gray-50">
                    <tr>
                        <th className="py-10 px-6 text-xl" onClick={handleSort}><FaSort className="inline-block mr-2"/>Id
                        </th>
                    ...
                    {users.map((data: any) => {
                        return (
                            <tr className="bg-white border-b" key={data.id}>
                                ...
                                <button name={data.id} onClick={e => handleEditUser(e, data)}
                                        className="inline-block mr-4 px-4 p-1 rounded bg-blue-600 text-md mt-6 text-indigo-50">
                                    <FaEdit className="inline"/>
                                    Edit
                                </button>
                                <button id={data.id} onClick={e => handleDeleteUser(e, data)}
                                        className="inline-block mr-4 px-4 p-1 rounded bg-red-600 text-md mt-6 text-indigo-50">
                ...
                <div className="w-full xl:bottom-28 bottom-20 absolute">
                    <button onClick={handleCreateUser} name="create"
                            className="bg-blue-800 text-white rounded-md text-lg px-6 p-2">
                        Create
                    </button>
                </div>
            </div>
